              ...
              return (
                ...
                >
                  <td className="py-4 px-4">
                    <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${isTop3 ? 'bg-indigo-500 text-white shadow-[0_0_15px_rgba(99,102,241,0.5)]' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                      {student.rank}
                    </span>
                  </td>
                  <td className={`py-4 px-4 font-medium ${isTop3 ? 'text-indigo-600 dark:text-indigo-300' : 'text-slate-800 dark:text-slate-200'}`}>
                    {student.name}
                  </td>
                  <td className="py-4 px-4 text-slate-500 dark:text-slate-400">{student.branch}</td>
                  <td className="py-4 px-4 font-bold text-slate-800 dark:text-slate-100">{parseFloat(student.cgpa).toFixed(2)}</td>
                  <td className="py-4 px-2 sm:px-4">
                    <span className={`whitespace-nowrap inline-flex items-center gap-0.5 px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-md text-[10px] sm:text-xs font-semibold border ${badge.color}`}>
                      {badge.label}
                    </span>
                  </td>
                </motion.tr>
